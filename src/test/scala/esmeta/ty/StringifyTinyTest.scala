...
class StringifyTinyTest extends TyTest {
  ...
  def init: Unit = {
    ...
    checkParseAndStringify("Ty", Ty)(
      ...
      PosInfinityT -> "+INF",
      NumberT -> "Number",
      NumberIntT -> "Integral[-0+]",
      NumberNonPosIntT -> "Integral[-0]",
      NumberNonNegIntT -> "Integral[0+]",
      NumberNegIntT -> "Integral[-]",
      NumberPosIntT -> "Integral[+]",
      (NumberNonNegIntT || NaNT) -> "Integral[0+, NaN]",
      NumberT(Number(Double.PositiveInfinity)) -> "Number[+INF]",
      NumberT(Number(Double.NegativeInfinity)) -> "Number[-INF]",
      NumberT(Number(Double.NaN)) -> "Integral[NaN]",
      NumberT(
