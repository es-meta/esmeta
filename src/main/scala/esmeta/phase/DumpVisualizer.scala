  ...
  def apply(cfg: CFG): Unit = {
    for {
      ...
      dumpJson(
        ...
        data = stepToNodeId.view.mapValues(_.toSet).toMap,
              ...
              thenNode.foreach(tn =>
                (node.loc, tn.loc) match
                  case (Some(l1), Some(l2))
                      if (l1.stepString == l2.stepString) =>
...
object DumpNodeIdToProgId {
  // directory that contains the most recent coverage dump generated by
  // `esmeta.es.util.Coverage` (symlink created during tests)
  private val RECENT_DIR = s"$TEST262TEST_LOG_DIR/recent"
    ...
    nodeIdToProgId.foreach {
      case (nodeId, featIdToProgId) =>
        dumpJson(
          ...
          data = featIdToProgId.view.mapValues(_.toMap).toMap,
...
object DumpNodeIdToTest262 {
  // directory for most-recent Test262 coverage dump
  private val RECENT_DIR = s"$TEST262TEST_LOG_DIR/recent"
...
class NodeCoverageJsonProtocol(cfg: CFG) {
  ...
  // Decoder for NodeJson (used by other decoders)
  ...
  // Decoder for NodeViewJson (requires NodeJson and ViewJson decoders above)
  ...
  // Decoder for NodeViewInfoJson (requires NodeViewJson decoder above)
