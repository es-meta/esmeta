...
sealed trait NumberTy extends TyElem with Lattice[NumberTy] {
  ...
  def canon: NumberTy = this match
    case NumberSignTy(sign, hasNaN) =>
      if sign.isZero then NumberIntTy(IntTy.Zero, hasNaN)
      else if sign.isBottom then NumberIntTy(IntTy.Bot, hasNaN)
      else this
...
object NumberTy extends Parser.From(Parser.numberTy) {
  ...
  lazy val Bot: NumberTy = NumberSignTy(Sign.Bot, false).canon
