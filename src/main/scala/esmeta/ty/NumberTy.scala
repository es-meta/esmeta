...
case class NumberTy(finite: FinNumberTy, inf: InfinityTy, nan: Boolean)
  ...
  def toNumberSet: Option[Set[Number]] =
    val NumberTy(fin, InfinityTy(pos), nan) = this.canon
    val specials =
      pos.map(p =>
        Number(if (p) Double.PositiveInfinity else Double.NegativeInfinity),
      ) ++
      (if (nan) Set(Number(Double.NaN)) else Set())
    fin.canon match
      case FinNumberSetTy(set) => Some(set ++ specials)
      case FinNumberIntTy(IntSetTy(set)) =>
        Some(set.map(n => Number(n.toDouble)) ++ specials)
      case f if f.isBottom => Some(specials)
