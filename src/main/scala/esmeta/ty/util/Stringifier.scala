...
object Stringifier {
  ...
  lazy val intRule: Rule[IntTy] = (app, ty) =>
    ty.canon match
  ...
  lazy val integralRule: Rule[(IntTy, Boolean)] = (app, t) =>
    val (ty, hasNaN) = t
    ty.canon match
      case ty if ty.isTop && hasNaN     => app >> "Integral"
      case ty if ty.isBottom && !hasNaN => app >> "Integral[Bot]"
      case IntSetTy(set) =>
        given Rule[Set[Long]] = setRule("", ", ", "")
        app >> "Integral[" >> set >> (if (hasNaN) ", NaN" else "") >> "]"
      case IntSignTy(sign) =>
        app >> "Integral[" >> sign >> (if (hasNaN) ", NaN" else "") >> "]"
  ...
  given numberTyRule: Rule[NumberTy] = (app, ty) =>
    ty.canon match
      case ty if ty.isTop    => app >> "Number"
      ...
      case NumberIntTy(int, hasNaN) =>
        given Rule[(IntTy, Boolean)] = integralRule
        app >> (int, hasNaN)
