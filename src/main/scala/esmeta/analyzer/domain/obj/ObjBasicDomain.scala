...
trait ObjBasicDomainDecl { self: Self =>
  ...
  object ObjBasicDomain extends ObjDomain {
    ...
    case class YetElem(tname: String, desc: String) extends Elem
    ...
    private def liftedAlpha(value: Value | Uninit): AbsOptValue = value match
      case v: Value  => AbsOptValue(AbsValue(v))
      case _: Uninit => AbsOptValue.Empty
    ...
    def alpha(obj: Obj): Elem = obj match
      ...
      case m @ MapObj(map) =>
        DetailMap(
          ...
          order = Some(map.keys.toVector.map(AValue.from)),
        )
      case ListObj(values)     => KeyWiseList(values.map(AbsValue(_)))
      case YetObj(tname, desc) => YetElem(tname, desc)
    ...
    given rule: Rule[Elem] = (app, elem) =>
      elem match
        ...
        case MergedMap(field, value) =>
          app >> "Map {{" >> field.toString >> " -> " >> value.toString >> "}}"
        case DetailMap(map, order) =>
          if (!map.isEmpty) app.wrap {
            order match
              case Some(order) =>
                for { (k, i) <- order.zipWithIndex.sortBy(_._2) }
                  app :> s"[$i] $k -> " >> map(k)
              case None => for { (k, v) <- map } app :> s"$k -> " >> v
          }
          else app >> "{}"
        case RecordElem(tname, map) =>
          app >> s"Record [$tname] " >> map
        case MergedList(value) =>
          app >> "List[[" >> value.toString >> "]]"
        case KeyWiseList(values) =>
          app >> values.mkString("List[", ", ", "]")
        case YetElem(tname, desc) =>
          app >> s"Yet[tname](" >> desc >> ")"
    ...
    extension (elem: Elem) {
      ...
      def ⊑(that: Elem): Boolean = (elem, that) match
        ...
        case (l: ListElem, r: ListElem) =>
          l.mergedValue ⊑ r.mergedValue
        case (YetElem(ltname, ld), YetElem(rtname, rd)) =>
          ltname == rtname && ld == rd
        case _ => false
        ...
        case (l: RecordElem, r: RecordElem) if l.tname == r.tname =>
          RecordElem(
            ...
            map = (l.map.keySet ++ r.map.keySet).toList
              .map(x => x -> AbsOptValue(elem(Str(x)) ⊔ that(Str(x))))
              .toMap,
          )
      ...
      def apply(key: AValue): AbsValue = elem match
        ...
        case m: DetailMap      => m.map.getOrElse(key, AbsValue.Bot)
        case MergedList(value) => value
        case RecordElem(_, map) =>
          key match
            case Str(field) => map.get(field).fold(AbsValue.Bot)(_.value)
            case _          => AbsValue.Bot
        case KeyWiseList(values) =>
          key match
            case Math(math) =>
            ...
            case _ => AbsValue.Bot
        case YetElem(_, desc) => AbsValue.Bot
      ...
      def get(akey: AbsValue): AbsValue = akey.getSingle match
        ...
        case Many =>
          elem match
            ...
            case r: RecordElem =>
              r.map.values.foldLeft(AbsValue.Bot)(_ ⊔ _.value)
      ...
      def reachableParts: Set[Part] = elem match
        ...
        case RecordElem(_, map) =>
          map.values.flatMap(_.value.reachableParts).toSet
        case MergedList(value) =>
      ...
      def update(field: AbsValue, value: AbsValue, weak: Boolean): Elem =
        ...
        def recordAux(key: String): RecordUpdater = _ match {
          case RecordElem(tname, map) =>
            RecordElem(tname, map + (key -> AbsOptValue(value)))
        }
        modifyMap(elem, field, aux, mergedAux, recordAux, weak)

      /** expand */
      def expand(field: AbsValue, weak: Boolean): Elem =
        def aux(key: AValue): MapUpdater = m => m
        def mergedAux: MapUpdater = m => m
        def recordAux(key: String): RecordUpdater = _ match {
          case RecordElem(tname, map) =>
            RecordElem(tname, map + (key -> AbsOptValue.Empty))
        }
        modifyMap(elem, field, aux, mergedAux, recordAux, weak)
