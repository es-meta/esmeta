...
class TypeErrorManager() {
  def init(analyzeErrors: Set[TypeError]) = pendingError =
    MSet() ++ analyzeErrors
  ...
  def addRuntimeErrors(
    runtimeErrors: MSet[DetailedTypeError],
    code: String,
  ): Unit = {
  ...
  def dump(baseDir: String): Unit = {
    mkdir(baseDir)
    lazy val orderedTotalError = totalError.toList
      .sortBy(_.id)
      .map(_.toString)
      .mkString(LINE_SEP + LINE_SEP)
    lazy val orderedRootError =
      rootError.toList.map(_.toString).mkString(LINE_SEP + LINE_SEP)
    lazy val orderedPendingError =
      pendingError.toList.map(_.toString).mkString(LINE_SEP + LINE_SEP)
    lazy val orderedVerifiedError = verifiedError.toList
      .sortBy(_.id)
      .map(_.toString)
      .mkString(LINE_SEP + LINE_SEP)
    lazy val orderedDiscoveredError = discoveredError.toList
      .sortBy(_.id)
      .map(_.toString)
      .mkString(LINE_SEP + LINE_SEP)
