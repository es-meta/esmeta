...
class Interpreter(
  ...
  val typeErrors: MSet[RuntimeTypeError] = MSet()
  private def addError(error: RuntimeTypeError): Unit = typeErrors += error
    ...
    case ret @ IReturn(expr) =>
      ...
      if (tyCheck)
        ...
        if (retTy.isDefined && !retTy.contains(retVal, st)) {
          ...
          addError(RuntimeTypeError(irp, retVal, st))
        }
      ...
      case (param :: pl, arg :: al) =>
        ...
        if (tyCheck)
          ...
          if (func.isMethod && idx == 0) ()
            ...
            addError(RuntimeTypeError(aap, arg, st))
        aux(pl, al)
    }
    aux(params, args)
    map
