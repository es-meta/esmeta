(** * Probe: universally quantified ToNumeric/ToNumber agreement — ITree route.

    ToNumericITreeProbe.v proved two ground facts (input [true] only).
    This file quantifies over the WHOLE input domain of two primitive
    types:

    - forall b : bool,  ToNumeric(b) and ToNumber(b) both yield the same
      number (1.0 for true, 0.0 for false) — by case split, each case a
      closed relational run.
    - forall f : float, ToNumeric(f) and ToNumber(f) both yield f itself
      — the interesting case: [f] stays SYMBOLIC through the whole run.
      This works iff every branch on the execution path tests only the
      type tag of the argument, never its value, so [repeat rstep] can
...
(** ** forall floats (ECMAScript Numbers) *)

Theorem tonumeric_tonumber_number :
  forall f : float, exists σ1 σ2,
    REval "probe" spec_fnames spec_lk (σ0 (VNumber f))
      (harness_tree "ToNumeric") (VNumber f) σ1 /\
    REval "probe" spec_fnames spec_lk (σ0 (VNumber f))
      (harness_tree "ToNumber") (VNumber f) σ2.
Proof.
  intros f; do 2 eexists; split; unfold REval; repeat rstep.
Qed.
