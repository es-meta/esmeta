...
[← Running a program](running.md) · [Transparent Builtins →](builtins.md)

These are the operations the rules are written against, and the specification
defines every one of them itself. A rule names one where ECMA-262 would name an
abstract operation, and its definition lives here so that the rule stays about
the thing it is about.

Operations the specification declares but does not define are on their own
page, [Transparent Builtins](builtins.md).

## Generic Operations

Lists, options, maps and sets, used everywhere below. A set is a braced
sequence and a map is a set of pairs, which is the shape the standard library
decodes.
...
Reading and writing the function table, the environments, the heap and the output stream.
...
Records, maps and lists, as they sit in the heap.
...
The same operations, reached through an address.
